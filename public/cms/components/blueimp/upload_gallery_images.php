 ...
 * http://www.opensource.org/licenses/MIT handle_file
 */
include ('../../config/config.php');

page_protect();
checkPermision($pdo, 'Gallery');

//abort
$handler = new UploadHandler($config, $prefix_table);
...
{   
    ...
    function __construct($config = null, $prefix_table = null, $options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'upload_dir' => '../../gallery/b/',
            'upload_url' => '../../gallery/b/',
    ...
    protected function handle_form_data($file, $index) {
		...
		if(isset($_POST['albumid'])){
			...
			$id = mysql_real_escape_string($_POST['albumid']);
		    $filename = $file->name;
			$name = $this->original_filename;
                        
                        // Ordering
                        
                        mysql_query("UPDATE `{$this->prefix_table}pics`
			...
			$dt=date("Y-m-d H:i:s");
			$sql="insert into  ".$this->prefix_table."pics  (albumid,filename,title,dt, `key_position`) values('$id','$filename','$name','$dt', 1) ";
			$result=mysql_query($sql); 
			if($result){
				$row=mysql_fetch_row(mysql_query("select max(id) from ".$this->prefix_table."pics"));
				$sql= q("UPDATE  ".$this->prefix_table."albums SET cnt =cnt +1,thumb = IF(thumb<>0,thumb,".$row[0].") WHERE id = '$id' ");
