...
* http://www.opensource.org/licenses/MIT handle_file delete
*/
...
class UploadHandler
{
	...
	function __construct($image_sizes = null, $options = null, $initialize = true, $error_messages = null)
	{

		//var_dump($this->get_full_url());
		$this->options = array(
			'script_url' => $this->get_full_url() . 'components/blueimp/',
			...
			'inline_file_types' => '/\.(pdf|doc?x|xls?x)$/i',
			...
			'image_versions' => array(
				...
				*/
				'thumb' => array( // Uncomment the following to force the max
						// dimensions and e.g. create square thumbnails:
					//'crop' => true,
					'max_width' => $image_sizes['thumb_width'], 'max_height' => 10000),

				'resize' => array( // Uncomment the following to force the max
						// dimensions and e.g. create square thumbnails:
					//'crop' => true,
					'max_width' => $image_sizes['resize_width'], 'max_height' => 10000),

				'square' => array(
					// Uncomment the following to force the max
					// dimensions and e.g. create square thumbnails:
					'crop' => true,
					'max_width' => $image_sizes['square_width'],
					'max_height' => $image_sizes['square_width'])));
		if ($options)
		{
	...
	protected function initialize()
	{
		switch ($this->get_server_var('REQUEST_METHOD'))
		{
			...
			case 'GET':
				...
				//Rotate Image
				if (isset($_GET['rotate']) && isset($_GET['img']))
				{

					$this->rotate($_GET['img'], $_GET['rotate']);
	...
	protected function get_upload_path($file_name = null, $version = null)
	{
		...
		//var_dump($this->options['upload_dir'].$this->get_user_path());
	...
	protected function create_scaled_image($file_name, $version, $options)
	{
		...
		$success = $src_img && @imagecopyresampled($new_img, $src_img, $dst_x, $dst_y, 0,
			0, $new_width, $new_height, $img_width, $img_height) && $write_image($new_img, $new_file_path,
			$image_quality);
	...
	{
		return array_key_exists($error, $this->error_messages) ? $this->error_messages[$error] :
			$error;
	}
	...
	{
		return $this->get_unique_filename($this->trim_file_name($name, $type, $index, $content_range),
			$type, $index, $content_range);
	}
	...
	protected function rename_file_unique($path, $filename)
	{


		$match = preg_match('/\.([a-zA-Z]{3,4})$/is', $filename, $found);

		$ext = strtolower($found[1]);

		$new_name = strtolower(md5($filename)) . '.' . $ext;
		...
		while (file_exists($path . $new_name))
		{
			...
			$new_name = strtolower(md5($filename)) . $x . '.' . $ext;
		...
		$index = null, $content_range = null)
	{
        ...
        $new_name = $this->rename_file_unique($this->get_upload_path($file->name), $name);
	...
	protected function generate_response($content, $print_response = true)
	{
		if ($print_response)
		{
			...
			if ($this->get_server_var('HTTP_CONTENT_RANGE'))
			{
				...
				{
					$this->header('Range: 0-' . ($this->fix_integer_overflow(intval($files[0]->size)) -
						1));
				}
			}
			$this->body($json);
	...
	//Rotate image
	private function rotate($img, $angle)
	{

		$file = new stdClass();
		$file->name = $img;
		$file_path = $this->get_upload_path($img);
		$this->rotateImage($file_path, $file_path, $angle);
	...
	private function rotateImage($sourceFile, $destImageName, $degreeOfRotation)
	{
