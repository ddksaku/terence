$(document).ready(function () {
	...
	$('#save').click(function(e){				
		  if(document.formnewpass.password.value == "" || document.formnewpass.password2.value == ""){
			  alertMessage("error","Please input the new password");
...
if(document.formnewpass.password.value != document.formnewpass.password2.value){
			  alertMessage("error","Password don't match");
			  $('.inner').jrumble({ x: 4,y: 0,rotation: 0 });	
			  $('.inner').trigger('startRumble');
			  setTimeout('$(".inner").trigger("stopRumble")',500);
			  return false;
...
if(document.getElementById("password").value.length < 6){
			  alertMessage("error","Password must have minimum 6 chars!");
			  $('.inner').jrumble({ x: 4,y: 0,rotation: 0 });	
			  $('.inner').trigger('startRumble');
			  setTimeout('$(".inner").trigger("stopRumble")',500);
			  return false;
	...
	$('form:eq(0)').submit(function(e){	
		loading('Checking ',1);
			...
			if(data.success != 1)
			{
                            alertMessage('error',"Invalid reset key!");
                            return false;
			}
