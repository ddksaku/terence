//Current Datatables page for table 4 menu
var TAB4_PAGE = 0;
...
$(document).ready(function(){
	...
	////////////////////////////////////////////////////////////
        /*
         *
         * This shouldn't be necessary anymore - so I'm temporarily commenting it out.
         *
	var security_check = false;
	$('.inner, #account_info').on('click', 'a', function(e){
		//If current check is not in progress
		if(!security_check) {
			//stop double checking
			security_check = true;

			//Check security for login
                        ajaxCallAPI(
                            'login',
                            'post',
                            'login',
			...
			$.post('components/security/security_check.php', {}, function(data){
	...
	////////////////////////////////////////////////////////////
	 ...
	 //Show hide button on fancybox error when ajax error occurs
	 $(document).ajaxError(function() {
		 
		 $('#fancybox-error').closest('#fancybox-outer').find('#fancybox-close').show();
  ...
  //Function to sort columns by checkboxes
  ...
  $('#dataTable').dataTable( {
	...
	//////////////////////////////////////////////////
	//Pages categories ordering
	//////////////////////////////////////////////////
	$('.pcreorder').live('click', function() {  
		  
		      	
			  var datavalue ='action=pcreorder&id='+$(this).attr("id")+"&move="+$(this).attr("name");                 	
			  $.ajax({
					  url: "pages/actions.php",
					  data: datavalue,
					  success: function(data){
								  $("#tab2 .load_page").fadeOut(500,function(){													   
												  if (data.check == "0")
												...
												}).load('pages/categoriesReload.php').fadeIn();	
										//ordering = false;		
										return false;
					  },	
					  cache: false,
	...
	$('.preorder').live('click', function() {  
			  ...
			  var datavalue ='action=preorder&id='+$(this).attr("id")+"&move="+$(this).attr("name");                 	
			  $.ajax({
					  url: "pages/actions.php",
					  data: datavalue,
					  success: function(data){
								  $("#tab1 .load_page").fadeOut(500,function(){													   
												  ...
												  $.fancybox.close();
												  setTimeout('unloading()',900);																											
												}).load('pages/pagesReload.php').fadeIn();	
										//ordering = false;		
										return false;
					  },	
					  cache: false,
	...
	//////////////////////////////////////////////////
	//News categories ordering
	//////////////////////////////////////////////////
	$('.reorder').live('click', function() {  
		  
		      	
			  var datavalue ='action=reorder&id='+$(this).attr("id")+"&move="+$(this).attr("name");                 	
			  $.ajax({
					  url: "news/actions.php",
					  data: datavalue,
					  success: function(data){
								  $("#tab2 .load_page").fadeOut(500,function(){													   
												  if (data.check == "0")
												  ...
												  $.fancybox.close();
												  setTimeout('unloading()',900);																											
												}).load('news/categoriesReload.php').fadeIn();	
										//ordering = false;		
										return false;
					  },	
					  cache: false,
	...
	//////////////////////////////////////////////////
	//Services categories ordering
	//////////////////////////////////////////////////
	$('.screorder').live('click', function() {  
		  
		      	
			  var datavalue ='action=screorder&id='+$(this).attr("id")+"&move="+$(this).attr("name");                 	
			  $.ajax({
					  ...
					  success: function(data){
								  $("#tab2 .load_page").fadeOut(500,function(){													   
												  ...
												  $.fancybox.close();
												  setTimeout('unloading()',900);																											
												}).load('services/categoriesReload.php').fadeIn();	
										//ordering = false;		
										return false;
					  },	
					  cache: false,
	...
	//Services ordering
	//////////////////////////////////////////////////
	$('.sreorder').live('click', function() {  
		  
		      	
			  var datavalue ='action=sreorder&id='+$(this).attr("id")+"&move="+$(this).attr("name");                 	
			  $.ajax({
					  url: "services/actions.php",
					  data: datavalue,
					  success: function(data){
								  $("#tab1 .load_page").fadeOut(500,function(){													   
												  ...
												  $.fancybox.close();
												  setTimeout('unloading()',900);																											
												}).load('services/servicesReload.php').fadeIn();	
										//ordering = false;		
										return false;
					  },	
					  cache: false,
	...
	***********************************************************************/  
	 ...
	 //Hidden All  Alert Message Before
	  ...
	  function alertMessage(type,str){
		    ...
		    $('.alertMessage').stop();
		    $('.alertMessage').animate({ opacity: 0,right: '30'}, 500,function(){ $(this).remove(); });
			
			// type is a success ,info, warning ,error
			$alert = $('<div id="alertMessage" class="alertMessage '+type+'">' +str+ '</div>');
			$('body').append($alert);
			$.alertbox = $alert;
			$.alertbox.show().animate({ opacity: 1,right: '10' },500,function(){
				
				$alertHolder = $(this);
				
				//Hide
				$alertHolder.delay(3000).animate({ opacity: 0,right: '30'}, 500, function(){ $(this).remove(); });
