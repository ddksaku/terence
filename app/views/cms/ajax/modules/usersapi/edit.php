                    ...
                    function(data)
                    {
                        if(data.success==0){   // uncomplete return 0
                                  ...
                                  if(data.error == 1){
                                    ...
                                    //
                                    $('input[name="email"]').validationEngine('showPrompt', 'Email already exists', 'error',"topLeft");
                                    //alertMessage("error","Duplicate email found, try again with other email");
                                  }else if(data.error == 2){
                                    //Error for duplicate username
                                    $('input[name="username"]').validationEngine('showPrompt', 'Username already exists', 'error',"topLeft");
  ...
  <div class="content">
			<form name="usersubmit" id="usersubmit-lightbox">
                            ...
                            <div class="section">
                                ...
                                <div>
                                    ...
                                    foreach ($groups as $group) {
                                        ...
                                        if (!$user->hasPermission('make_level_'.$group->group_level) && !$in_group) {
                                            continue;
                                        }
                                        
                                        if ($edit_user->user_id == $user->user_id && !$in_group) {
                                            continue;
                                        }
                 ...
                 <div class="section">
                      ...
                      <input type="password" name="passwordCon" id="passwordCon" class="validate[<?php if ($new_user): ?>required,<?php endif; ?>equals[password]] medium" /></div>
				 </div>
				 <div class="section">
				 ...
				 <div class="section last">
					  <div><a class="btn btn-success sendform-lightbox">
                                                  <?php if ($new_user): ?>
                                                  Submit
                                                  <?php else: ?>
                                                  Update
